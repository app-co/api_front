...
export async function generateQrCode(id: number) {
  const gerencianet = new Gerencianet(options)
...
export async function createWebhook(urlNotification: string, chavePIX: string) {
  const body = {
...
export async function detailsWebhookPix() {
  const params = {
    chave: process.env.GN_PIX
  }
...
export async function webhookPix() {
